...
async function runTradingEngine() {
    console.log('--- STARTING TRADING ENGINE RUN ---');

    // Ensure wallet exists
    let wallet = await prisma.wallet.findFirst();
    if (!wallet) {
        wallet = await prisma.wallet.create({
            data: { cashBalance: 1000000.0 } // 1 Million initial paper cash
        });
    ...
    // 2. Fetch new Top 10 Stocks
    console.log('Fetching new Top 10 from Screener...');
    const newTop10 = await getTop10Stocks();
