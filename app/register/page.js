...
import { TrendingUp, KeyRound, Mail, User } from 'lucide-react';
...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!error) {
                toast.success('Account created! Check your email to verify (if enabled).');
                // You might need an API call to insert into your public.User table as well, 
                // or use a Supabase Database Webhook/Trigger on `auth.users` insert.
                // For now, if auto-login is enabled, we'll try to redirect.
                if (data.session) {
                ...
                <div className="bg-white dark:bg-slate-800 py-8 px-4 shadow-xl sm:rounded-xl sm:px-10 border border-slate-100 dark:border-slate-700">
                    <form className="space-y-6" onSubmit={handleSubmit}>
                        ...
                        <div>
                            ...
                            >
                                {isLoading ? 'Creating Sandbox...' : 'Register'}
                            </button>
                        </div>
                    </form>
                </div>
